...
<script>
	...
	function fnBtnBuscar() {
		...
		let url = "CategoryBuscar?names=" + names;
	...
	function fnBtnActualizar() {
		let xhttp = new XMLHttpRequest();
		xhttp.open("GET", "CategoryActualizar", true);
		xhttp.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
				let respuesta = xhttp.responseText;
				arreglo = JSON.parse(respuesta);
	...
	function fnEstadoFormulario(estado){
		...
		frmDescripcion.disabled = (estado==ACCION_ELIMINAR)
	...
	function exportToExcel() {
        ...
        data.push(["ID", "NOMBRE", "DESCRIPCIÓN"]);
        rows.forEach(function(row) {
          let rowData = [];
          let columns = row.querySelectorAll("td:nth-child(1), td:nth-child(2), td:nth-child(3)"); // Incluir solo las columnas deseadas
        ...
        XLSX.writeFile(workbook, "reporteCategorias.xlsx");
  	}
	...
	function exportToCSV() {
	  ...
	  let filename = "reporteCategorias.csv";
	...
	function exportToPDF() {
		...
		const docDefinition = {
			content: [
				{ text: 'Reporte PDF - Categorias', style: 'header' },
				{
					table: {
						headerRows: 1,
		...
		pdfMake.createPdf(docDefinition).download('reporteCategorias.pdf');
	}
</script>
</body>
</html>